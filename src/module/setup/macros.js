export class ArchmageMacros {
  ...
  static async aasimarHalo(speaker, actor, token, character, archmage) {
    ...
    const effects = [
      { key: "system.attributes.ac.value", value: 2, type: CONST.ACTIVE_EFFECT_MODES.ADD },
      { key: "system.attributes.pd.value", value: 2, type: CONST.ACTIVE_EFFECT_MODES.ADD },
      { key: "system.attributes.md.value", value: 2, type: CONST.ACTIVE_EFFECT_MODES.ADD }
    ];
  ...
  static async barbarianWhirlwind(speaker, actor, token, character, archmage) {
    ...
    let effectData = {
      ...
      icon: archmage.item.img,
      changes: [
        { key: "system.attributes.ac.value", value: penalty, type: CONST.ACTIVE_EFFECT_MODES.ADD },
        { key: "system.attributes.pd.value", value: penalty, type: CONST.ACTIVE_EFFECT_MODES.ADD }
      ]
    }
  ...
  static async clericHammerOfFaith(speaker, actor, token, character, archmage) {
    ...
    const effectData = {
      ...
      changes: [{
        ...
        value: bonus ,
        type: CONST.ACTIVE_EFFECT_MODES.OVERRIDE
      }]
    };
    game.archmage.MacroUtils.setDuration(effectData, CONFIG.ARCHMAGE.effectDurationTypes.EndOfCombat);
    await actor.createEmbeddedDocuments("ActiveEffect", [effectData]);
  ...
  static async fighterCarveAnOpening(speaker, actor, token, character, archmage) {
    ...
    let effectData = {
      ...
      icon: archmage.item.img,
      changes: [
        { key: "system.attributes.critMod.atk.value", value: bonus + prev, type: CONST.ACTIVE_EFFECT_MODES.ADD },
      ]};
    game.archmage.MacroUtils.setDuration(effectData, CONFIG.ARCHMAGE.effectDurationTypes.EndOfCombat);
    await actor.createEmbeddedDocuments("ActiveEffect", [effectData]);
  }
  ...
  static async fighterDefensiveFighting(speaker, actor, token, character, archmage) {
    ...
    if (is2e) {
      ...
      // Apply to AC.
      effects.push({ key: "system.attributes.ac.value", value: bonus, type: CONST.ACTIVE_EFFECT_MODES.ADD });
      // Champion and/or Epic feat applies to PD.
      if (feats.champion || feats.epic) {
        effects.push({ key: "system.attributes.pd.value", value: bonus, type: CONST.ACTIVE_EFFECT_MODES.ADD });
      }
      // Epic feat applies to MD.
      if (feats.epic) {
        effects.push({ key: "system.attributes.md.value", value: bonus, type: CONST.ACTIVE_EFFECT_MODES.ADD });
      }
    }
    else {
